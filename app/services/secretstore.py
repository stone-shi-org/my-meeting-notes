"""Encryption for per-user integration credentials at rest.

Integration secrets are a different class of thing from the MCP tokens that came
before them: a Google refresh token or an Apple app-specific password grants
standing access to somebody's real mailbox and calendar. Those get encrypted with
Fernet so that a copy of ``data/app.db`` is not a copy of the user's accounts.

Be honest about the threat model. Unless ``MMN_SECRET_KEY`` is set the key lives in
the same bind-mounted volume as the database, so this protects a stolen database
dump and not much else. That is still worth having, because a dump is by far the
most likely way the file leaves the box.

Named ``secretstore`` rather than ``secrets`` so it cannot be confused with the
stdlib module of that name, which ``app/security.py`` uses for token minting.
"""
...
class SecretDecryptError(AppError):
    ...
    Callers must catch this and mark the integration ``reauth_required``. A lost
    key has to degrade to "reconnect your accounts", never to a 500.
    """
...
_cached: tuple[str, bytes] | None = None


def key_path() -> Path:
    return Path(get_settings().data_dir) / KEY_FILENAME


def _normalise(material: str) -> bytes:
    """Accept a real Fernet key verbatim, else derive one from any string.

    Requiring base64 from whoever sets ``MMN_SECRET_KEY`` is a footgun -- the
    error surfaces as an unrelated-looking ValueError deep inside Fernet. So a
    well-formed key is used as-is and anything else is hashed into the 32 bytes
    Fernet needs.
    """
    raw = material.strip()
    try:
...
def load_key() -> tuple[str, bytes]:
    ...
    Returns ``(key_id, key)``. ``key_id`` is recorded alongside each ciphertext so
    a future key rotation can tell which key a given blob belongs to.
    """
    global _cached
    ...
    path = key_path()
    env_material = (get_settings().secret_key or "").strip()
    ...
    if env_material and file_material and _normalise(env_material) != _normalise(file_material):
        raise SecretKeyError(
            f"MMN_SECRET_KEY is set, and {path} holds a different key. Remove whichever "
            "one is stale -- silently picking either would make every credential "
            "encrypted under the other permanently unreadable."
        )
    ...
    if env_material:
        ...
        with os.fdopen(fd, "w") as fh:
            fh.write(material)
        log.warning(
            "generated a new credential encryption key at %s -- back it up; losing it "
            "means every connected account must be reconnected",
            path,
        )
...
def decrypt(blob: str | None) -> dict:
    ...
    try:
        ...
        raise SecretDecryptError(
            f"Stored credential could not be decrypted.{hint}"
        ) from exc
