...
Session tokens are opaque and stored hashed: the database holds
``sha256(token)`` while the raw token only ever lives in the cookie. A dump of
``sessions`` therefore yields nothing an attacker can replay.
"""
...
def hash_password(password: str, params: ScryptParams | None = None) -> dict[str, str]:
...
def hash_token(token: str) -> str:
    """The ``sessions.id`` for a raw token."""
...
def validate_password(password: str, *, min_length: int, current: str | None = None) -> str | None:
